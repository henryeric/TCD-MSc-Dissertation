...
set.seed(123)
data <- data.frame(
  variable = rep(c("gender", "time_of_day", "region"), each = 2),
...
main_plot <- ggplot(data, aes(x = tendency, y = reorder(variable, -tendency), 
  ...
  scale_x_continuous(limits = c(1, 8), 
                     breaks = c(1, 7),
                     labels = c("jumpy tendency", "smooth tendency")) +
  labs(x = "", y = "") +
  theme_minimal() +
  theme(
...
#
title <- ggdraw() + 
  draw_label("The tendency of variable categories towards buffering time", 
