...
set.seed(123)
data <- data.frame(
  variable = rep(c("gender", "content_language", "content_genre", "age", "buffer"), each = 2),
  category = c("female", "female",
               "French", "Spanish/German", 
               "documentary", "sci-fi/drama",
               "middle", "young/old",
               "more", "less"
  ),
  tendency = c(3.5, 6.2,   
...
main_plot <- ggplot(data, aes(x = tendency, y = reorder(variable, -tendency), 
  ...
  scale_x_continuous(limits = c(1, 8), 
                     breaks = c(1, 7),
                     labels = c("low tendency", "high tendency")) +
  labs(x = "", y = "") +
  theme_minimal() +
  theme(
...
#
title <- ggdraw() + 
  draw_label("The tendency of variable categories towards content_completion_rate", 
