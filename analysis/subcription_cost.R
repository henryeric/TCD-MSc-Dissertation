...
y <- as.factor(data$subscription_cost)
...
#x_factors <- model.matrix(~ as.factor(viewing_device) + as.factor(state) + as.factor(content_genre) + as.factor(content_type) + as.factor(age_group) + as.factor(gender) + as.factor(time_of_day) + as.factor(watching_with_others) + as.factor(content_language) + as.factor(user_feedback) + as.factor(binge_watching) + as.factor(download_quality) + as.factor(network_type) + as.factor(account_type) + as.factor(promotions_used), data = data)
